...
func TestOnlineTicTacToeFlow(t *testing.T) {
	...
	if err != nil || joined.Status != "active" || joined.You != "o" {
	...
	for _, move := range []struct {
		...
		if _, err := m.move(waiting.Code, move.id, move.cell); err != nil {
			t.Fatal(err)
		}
	}
	finished, err := m.state(waiting.Code, first.ID)
	if err != nil || finished.Status != "finished" || finished.Winner != "x" {
		t.Fatalf("winner missing: %#v %v", finished, err)
	}
}
